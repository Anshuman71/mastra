...
// Define your subagents with clear, specific descriptions
...
const stream = await supervisorAgent.stream('Find me the weather in Tokyo.', {
  ...
  delegation: {
    // Control what context is shared with subagents
...
1. **When to delegate** - Does this task require a subagent?
1. **Which agent to use** - Which subagent is best suited for this subtask?
1. **What to pass** - What context and instructions should the subagent receive?
1. **When to stop** - Is the task complete, or should it continue iterating?

### Best practices for supervisor instructions

**Be specific about each subagent's role:**
...
### subagent descriptions matter too

Each subagent should have a clear `description` that helps the supervisor understand when to use it:
...
- [ ] **Add descriptive `description` fields to all subagents**
