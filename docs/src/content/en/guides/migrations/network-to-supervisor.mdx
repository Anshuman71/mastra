...
Control and modify subagent delegations:
...
const stream = await supervisorAgent.stream('Research AI in education', {
  ...
  delegation: {
    // Called before delegating to a subagent or workflow
...
Control what conversation context is shared with subagents:
...
const stream = await supervisorAgent.stream('Research AI in education', {
  ...
  delegation: {
    // Filter context passed to subagents
...
One key improvement in the supervisor pattern is **memory isolation** - subagents receive the full conversation context for better decision-making, but only their specific delegation prompt and response are saved to their memory.
...
// ✅ subagent can see "Alice" and "Paris" in conversation
// ✅ Only delegation prompt + response saved to subagent memory
// ✅ subagent memory stays clean, not polluted with supervisor context

const stream = await supervisorAgent.stream('What is my name and where do I live?', {
  maxSteps: 10,
  delegation: {
    // Each subagent invocation uses a fresh unique thread
    // Full context is forwarded for decision-making
    // But only the delegation is persisted to subagent memory
  },
})
```

This ensures subagents:
...
```typescript
// subagent with requireApproval tool
...
1. **When to use each resource**: Clear criteria for delegation
1. **How to coordinate**: Order of operations, dependencies
1. **Success criteria**: When the task is complete
...
Each subagent should also have a clear description:
...
- [ ] **Add `description` fields to all subagents**
