...
By default, subagents receive the full conversation context from the supervisor. Use `contextFilter` to control what messages are shared -- for example, to remove sensitive data or limit the number of messages:
...
Completion scorers automatically validate whether the task is complete. If validation fails, the supervisor continues iterating. The feedback from failed scorers is included in the conversation context so subagents can see what was missing:
