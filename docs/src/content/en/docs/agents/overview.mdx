...
You can register subagents within an agent using the `agents` property. Adding subagents makes the parent agent a supervisor — it can delegate tasks to subagents, control delegations with hooks, filter context, and monitor progress using the [supervisor pattern](/docs/agents/supervisor-agents). Read the [using agents as tools](/docs/agents/using-tools#using-agents-as-tools) documentation for setup details.
