...
When using the supervisor pattern (multi-agent coordination via `.stream()` or `.generate()`), tool approvals properly propagate through the delegation chain. This means when a supervisor agent delegates to a subagent that uses a tool requiring approval, the approval request surfaces to the supervisor level where it can be handled.

### How it works

1. Supervisor delegates to subagent
1. subagent calls a tool with `requireApproval: true` or `suspend()`
1. Approval request bubbles up through delegation chain
1. You handle the approval at the supervisor level
1. Approval/decline propagates back down to the subagent
1. subagent continues or terminates based on approval decision
...
User Request
  → Supervisor Agent
    → subagent A
      → subagent B (has tool with requireApproval: true)
