...
1. Sub-agent calls a tool with `requireApproval: true` or `suspend()`
1. Approval request bubbles up through delegation chain
1. You handle the approval at the supervisor level
1. Approval/decline propagates back down to the sub-agent
1. Sub-agent continues or terminates based on approval decision
