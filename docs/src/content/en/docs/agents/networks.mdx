...
### Task completion feedback visibility

When using task completion scorers (for task validation), the completion feedback is automatically included in the conversation context passed to subagents on subsequent iterations. This allows subagents to see what validation failed and adjust their responses accordingly.
...
// Task completion check runs: FAIL (missing recommendations)
// Task completion feedback added to messages: "Task not complete. Missing: recommendation"
