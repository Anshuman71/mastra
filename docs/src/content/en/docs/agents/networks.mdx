...
The supervisor pattern using `.stream()` or `.generate()` is the recommended approach for multi-agent coordination. It provides delegation hooks, iteration monitoring, message filtering, and task completion scoring for fine-grained control over execution.

See [Supervisor Agents](./supervisor-agents) for full documentation on delegation hooks, message filtering, iteration monitoring, and more.
